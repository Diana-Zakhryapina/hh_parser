...
async def button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'search':
        ...
        await start_with_search(update, context)
...
async def start_with_search(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if name or salary or experience_id:
        keyboard.append([
    ...
    if response.status_code == 200:
        ...
        if vacancies:
            ...
            for vacancy in vacancies:
                ...
                cursor.execute(
                    '''
                    INSERT INTO vacancies (vacancy_id, title, professional_roles, employer, salary_from, salary_to, currency, experience, employment, area, metro_stations, url, responsibility)
...
async def filter_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not area:
        keyboard.append([InlineKeyboardButton("Город", callback_data='area')])
    if not metro:
        keyboard.append([InlineKeyboardButton("Метро", callback_data='metro')])
    if not employment:
        keyboard.append([InlineKeyboardButton("Занятость", callback_data='employment')])

    if currency or metro or employment or area:
        keyboard.append([
...
async def reset_filters(update: Update, context: ContextTypes.DEFAULT_TYPE):
    keys_to_clear = ['currency', 'metro', 'employment', 'area']
